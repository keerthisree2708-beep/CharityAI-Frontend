...
import { BookOpen, MapPin, Calendar, Camera, CheckCircle2 } from "lucide-react";

export function DonateBooks() {
  ...
  if (isSubmitted) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-6 animate-in zoom-in-95">
        ...
        <div>
          <h1 className="text-3xl font-bold mb-2">Book Donation Registered!</h1>
          <p className="text-[var(--brand-grey-dark)] max-w-md mx-auto">
            You are helping spread knowledge! A pickup will be scheduled soon.
          </p>
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div className="flex items-center gap-4 border-b border-[var(--border-color)] pb-6">
        <div className="p-4 bg-purple-50 dark:bg-purple-900/30 text-purple-500 rounded-2xl">
          <BookOpen size={32} />
        </div>
        <div>
          <h1 className="text-3xl font-bold text-[var(--text-color)]">Donate Books & Edu Kits</h1>
          <p className="text-[var(--brand-grey-dark)]">Empower the next generation with educational resources.</p>
        </div>
      </div>
        ...
        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setIsSubmitted(true); }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">Book Titles / Grades</label>
                <textarea 
                  className="input-field h-32 py-2" 
                  placeholder="E.g., 15 Grade 5 Math textbooks, 20 Notebooks..."
                  required
                ></textarea>
              </div>
              <div>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-[var(--border-color)] rounded-2xl h-[264px] flex flex-col items-center justify-center text-center p-6 hover:bg-gray-50 dark:hover:bg-[#1A1A1A] transition-colors cursor-pointer">
                ...
                <p className="text-xs text-[var(--brand-grey-dark)] mt-1">JPEG, PNG up to 5MB.</p>
              </div>
            </div>
          </div>

          <div className="pt-6 border-t border-[var(--border-color)] flex justify-end">
            <button type="submit" className="btn-primary w-full py-4 text-lg bg-purple-600 hover:bg-purple-700 shadow-xl shadow-purple-500/20">
